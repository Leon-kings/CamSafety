...
import { SearchOrdersModal } from "../search/OrderSearch";
import {
  Menu as MenuIcon,
  ChevronLeft,
  ChevronRight,
...
const API_URL = import.meta.env.VITE_API_URL || "https://camera-safety.onrender.com/api/orders";

const orderService = {
  getOrders: async (search = "", status = "") => {
    try {
  ...
  createOrder: async (orderData) => {
    try {
      const response = await axios.post(API_URL, orderData, { timeout: 8000 });
...
export const OrderManagement = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (currentOrder._id) {
        await orderService.updateOrder(currentOrder._id, currentOrder);
      } else {
  ...
  return (
    ...
    <div className="min-h-screen mt-4 rounded-2xl bg-gray-50 p-2 sm:p-4 md:p-6">
      ...
      <div className="sm:hidden flex items-center justify-between mb-4">
        <h3 className="text-xl font-bold text-black">Orders</h3>
        <button
          onClick={() => openModal()}
      ...
      <div className="flex flex-wrap gap-5 mb-5">
        <SearchOrdersModal API_URL={API_URL} />
        ...
        <>
          ...
          <div className="sm:hidden space-y-4">
            {orders.length > 0 ? (
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-xs text-black">
                        {order.orderDetails.planName} ({order.orderDetails.cameraCount} cameras)
                      </p>
                    </div>
                    ...
                    <p className="text-sm text-black">
                      ...
                      {order.orderDetails.discountAmount > 0 && (
                        <span className="text-xs text-green-600 ml-2">
                          (Saved ${order.orderDetails.discountAmount.toFixed(2)})
                        </span>
                      )}
                    </p>
                    <p className="text-xs text-black">
                  ...
                  <div className="mt-4 flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      {order.payment.status !== "completed" && (
                        <button
                          onClick={() => handleStatusChange(order._id, order.payment.status)}
              ...
              <div className="text-center py-8 text-black">No orders found</div>
            )}
          </div>
          ...
          <div className="hidden sm:block overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  {renderTableCell("Order ID", "ID", true)}
                  {renderTableCell("Customer", "Customer", true)}
                  {renderTableCell("Plan", "Plan", true)}
                ...
                {orders.length > 0 ? (
                  orders.map((order) => (
                    <motion.tr
                      ...
                      animate={{ opacity: 1, y: 0 }}
                    >
                      {renderTableCell("Order ID", order._id.substring(18, 24))}
                      {renderTableCell("Customer", (
                      ...
                      {renderTableCell("Plan", `${order.orderDetails.planName} (ID: ${order.orderDetails.planId})`)}
                      {renderTableCell("Cameras", order.orderDetails.cameraCount)}
                      {renderTableCell("Price", (
                        <>
                          ${order.orderDetails.finalPrice.toFixed(2)}
                          {order.orderDetails.discountAmount > 0 && (
                      ...
                      {renderTableCell("Date", format(new Date(order.createdAt), "MMM dd, yyyy HH:mm"))}
                      <td className="px-2 py-4 whitespace-nowrap">
                        <div className="flex gap-2">
                          <button
                            onClick={() => openModal(order)}
                  ...
                  <tr>
                    <td colSpan="8" className="px-4 py-6 text-center text-black">
                      No orders found
                    </td>
                  </tr>
                )}
            ...
            >
              <div className="p-6">
                <h2 className="text-xl font-bold mb-4 text-black">
                  {currentOrder._id
                    ? `Edit Order (ID: ${currentOrder._id.substring(0, 8)}...)`
                    : "Create New Order"}
                </h2>
                
                {/* Customer Information */}
                <div className="mb-6">
                  <h3 className="text-lg font-medium mb-4 text-black">Customer Information</h3>
                ...
                {/* Order Details */}
                <div className="mb-6">
                  <h3 className="text-lg font-medium mb-4 text-black">Order Details</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                      <label className="block text-sm font-medium text-black mb-1">Plan ID *</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-black mb-1">Camera Count *</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-black mb-1">Final Price *</label>
                      <input
                        type="number"
                ...
                {/* Features */}
                <div className="mb-6">
                  <h3 className="text-lg font-medium mb-4 text-black">Features</h3>
                  <div className="flex gap-2 mb-3">
                    <input
                      type="text"
                      placeholder="Add Feature"
                      value={currentOrder.newFeature || ""}
                      onChange={(e) => setCurrentOrder(prev => ({ ...prev, newFeature: e.target.value }))}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {currentOrder.orderDetails.features.map((feature, index) => (
